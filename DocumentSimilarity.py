...
class DocumentSimilarity:
  ...
  def remove_stop_words(self, word_list):
    ...
    return  [x for x in word_list if x not in self.stop_word_list]
  ...
  def string_tokenizer(self, text):
    ...
    line_list = text
    word_list = preprocess_text(line_list) 
    stop_word_removed_list = remove_stop_words(word_list)
    freq_mapping = word_frequency(stop_word_removed_list) 
  ...
  def cosine_similarity(self, sorted_word_dict_one, sorted_word_dict_second):
    ...
    numerator = dictionary_dot_product(sorted_word_dict_one, sorted_word_dict_second) 
    denominator = math.sqrt(dictionary_dot_product(sorted_word_dict_one,
      sorted_word_dict_one)*dictionary_dot_product(sorted_word_dict_second, sorted_word_dict_second)) 
  ...
  def similarity_score(self, text_first, text_second):
    ...
    distance = 1 - (cosine_similarity(sorted_word_dict_one, sorted_word_dict_second) / math.acos(0))
